...
(function (root, factory) {
    ...
    if (action.type === 'PLAY_CARD') {
      ...
      for (const pt of blastPoints) {
        if (pt.x >= 0 && pt.x < 20 && pt.y >= 0 && pt.y < (match.GH || 20)) {
          ...
          if (opp && opp.fleet) {
            for (const s of opp.fleet) {
              if (s.alive && s.cells && s.cells.some(c => c.x === pt.x && c.y === pt.y)) {
                s.hp = Math.max(0, s.hp - pt.dmg);
                const hitEntry = { x: pt.x, y: pt.y, dmg: pt.dmg, shipId: s.id };
                match.hits[playerId].push(hitEntry);
                hitDetails.push(hitEntry);
                hitCount++;
                if (s.hp <= 0) {
                  s.alive = false;
                  if (s.id === 'flagship') {
  ...
  function end1v1Turn(match, playerId) {
    if (!match) return;
    if (match.activePlayerId !== playerId) return;

    const nextId = playerId === 'p1' ? 'p2' : 'p1';
    match.activePlayerId = nextId;
    const nextPlayer = match.players.find(p => p.id === nextId);
    if (nextPlayer) {
      nextPlayer.cp = Math.min(nextPlayer.maxCp || 6, nextPlayer.cp + 2);
      while (nextPlayer.hand.length < 5 && nextPlayer.deck && nextPlayer.deck.length > 0) {
        nextPlayer.hand.push(nextPlayer.deck.pop());
      }
    }
    match.round = (match.round || 1) + (nextId === 'p1' ? 1 : 0);
  }
